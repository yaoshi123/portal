...
<head>
    ...
    <script type="text/javascript" charset="utf-8">
                    ...
                    if (data.status) {
                        ...
                        for (var i = 0; i < 10; i++) {
                            if (data.msg.array[i] != null) {
                                var status, showCheck, reportStatus, operaction, fileOperaction;
                                ...
                                if (data.msg.array[i].bpm_status == '0') {
                                    ...
                                    fileOperaction = 'style="display: inline-block;"';
                                ...
                                $("#reportOperation").append(
                                        '<form id="form' + i + '"action="{{url('index/upload')}}" method="post"enctype="multipart/form-data">' +
                                        '<a href="javascript:;" '+fileOperaction+' onclick = downloadFile(' + i + ');>下载附件</a> ' +
                                        '<input type="file" '+fileOperaction+'class = "inputFile" name="file" id="file" /> {{csrf_field()}}' +
                                        '<input name="reportId" type="hidden" value="' + data.msg.array[i].report_num + '"/>' +
                                        '<a href="javascript:;"  '+fileOperaction+' onclick = imgSubmit(' + i + ');>确认上传</a> ' +
                                        '<a href="javascript:;" onclick="showImg(' + i + ')" ' + showCheck + '>查看</a></form>');
        ...
        function showImg(data){
            var url = $("#imgUrl"+data).html()
            if(url){
                $("#img").attr({"src":$("#imgUrl"+data).html()});
                $('#hide-div').css({left:0})
            }
        }
        //翻页
        ...
        function edit(data){
            ...
            window.location.href = "/unit/appraisereport";
        }
        //显示相关信息
        function show(){
        ...
        function checkReportInfo(data){
            var reportNum = $("#reportCode p:eq("+(data + 1)+")").html();
            window.sessionStorage.setItem('reportNum',reportNum);
            window.sessionStorage.setItem('showReport',true);
            window.location.href = "/unit/appraisereport";
        }
        //下载已经有信息的Word文档
        ...
        <div class="appraisal-tab">
            ...
            <div class="appraisal-con">
                <ul>
                    ...
                    <li class="appraisal-con-tit" id="reportOperation"/>
